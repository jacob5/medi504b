...
The dataset consists of physical and clinical parameters collected from 10 hospitals across Kerala, India, and from 541 women, to determine PCOS and infertility-related issues. The dataset contains information that can be used to analyze and understand the diagnosis and treatment of PCOS and infertility. The full list of variables and the dataset may be found in the link provided in the references section. 

## 2. Objectives

Our objectives were twofold: 

1: Develop a simple model that can predict PCOS status using data that can be acquired using a routine blood test and/or assessment by a general practitioner clinician. 
...
To assess the relative fit and performance of these three models, we started with a simple logistic regression method. We fit the three models described above and compared the Akaike Information Criteria (AIC) as well as the Area Under the Receiving Operator Characteristic (AUC). Following this initial selection, we selected the two models with the best balance between both of these parameters, as well as clinical utility and applied various machine learning training methods to optimize model performance. We used the caret package to implement our various machine learning models. 

We applied regularization methods, boosting methods, as well as random forest modelling using the variable combinations identified using the simple logistic regression approach. 

This left us with a 6-way comparision: compared the performance of two different variable combinations (clinical only vs. clinical and physiologic) across three different machine learning methodologies (LASSO vs. Random Forest vs. XGBoost) to estimate the outcome of interest. 

The rationale for testing both models with machine learning optimization was to assess if a simpler model with clinical parameters only could approximate the classification performance of a more complex model. The final model was selected based on a balance of accuracy, specificity, and clinical ease of use. 
...
### Table 1: Clinical and sociodemographic characteristics of the study cohort (n=541, women from Kerala, India) stratified by PCOS status
...
tableData <-  PCOS_data_without_infertility_formatted %>%
  mutate(
"Age"=age_yrs,              
"BMI"=bmi,                  
"Pulse rate"=pulse_rate_bpm,      
"Respiratory rate"=rr_breaths_min,       
"Hemeglobin (g/dl)"=hb_g_dl,              
"Pregnancy status"=pregnant_y_n,        
"Human chorionic gonadotropin (IU/ml)"=as.numeric(i_beta_hcg_m_iu_m_l),  
"Follicle stimulating hormone (IU/ml)"=as.numeric(fsh_m_iu_m_l),         
"Lutenizing hormone (IU/ml)"=as.numeric(lh_m_iu_m_l),          
"Thyroid stimulating hormone (IU/ml)" = as.numeric(tsh_m_iu_l),           
"Anti-mullerian hormone (ng/ml)"=as.numeric(amh_ng_m_l),           
"Prolactin (ng/ml)"=as.numeric(prl_ng_m_l),          
"Vitamin D (ng/ml)" = as.numeric(vit_d3_ng_m_l),        
"Progesterone (ng/ml)" = as.numeric(prg_ng_m_l),          
"Random blood sugar (mg/dl)"=as.numeric(rbs_mg_dl),           
"Recent weight gain"=weight_gain_y_n,      
"Recent hair growth"=hair_growth_y_n,      
"Recent skin darkening"=skin_darkening_y_n,  
"Recent hair loss"=hair_loss_y_n, 
"Recent onset acne"=pimples_y_n)%>%
mutate(`Recent weight gain`=ifelse(`Recent weight gain`==1, "Yes", "No"), 
`Recent hair growth`=ifelse(`Recent hair growth`==1, "Yes", "No"), 
`Recent skin darkening`=ifelse(`Recent skin darkening`==1, "Yes", "No"), 
`Recent hair loss`=ifelse(`Recent hair loss`==1, "Yes", "No"), 
`Recent onset acne`=ifelse(`Recent onset acne`==1, "Yes", "No"), 
`Pregnancy status`=ifelse(`Pregnancy status`==1, "Yes", "No"))
  
tab1.1<-table1::table1(~
`Age`+
`BMI`+
`Pulse rate`+
`Respiratory rate`+
...
`Lutenizing hormone (IU/ml)`+
`Thyroid stimulating hormone (IU/ml)`+
...
`Recent weight gain`+
`Recent hair growth`+
`Recent skin darkening`+
`Recent hair loss`+
`Recent onset acne`
|
  as.factor(PCOS_Status), 
data=tableData)%>%as.data.frame()
...
From table 1, we can observe some qualitative differences among women with and without PCOS in the following parameters: 
...
#### Figure 1. Plotting categorical associations for clinical variables deemed eligible for modelling (clinical variables easily determined during a routine clinical examination)



```{r EDA4, echo=FALSE, fig.height=10, fig.width=20, message=FALSE, warning=FALSE, results= "hide", fig.align = 'center'}
...
From the figure, we can see that the proportion of individuals with weight gain and a positive PCOS status is greater than the proportion of individuals with no weight gain and a positive PCOS status. We observe similar associations for hair growth and skin darkening. We also observe a similar but much weaker association for the categorical variables of hair loss and pimples. As these predictors appear to be visually associated with the outcome, as well as physiologically feasible to be associated with PCOS, they should be considered for inclusion in the predictive model. 
...
#### Figure 2. Boxplots looking at relationship of selected physiologic parameters and PCOS status (note log-scale on y-axis)
```{r hormonevariables, echo=FALSE, fig.height=12, fig.width=20, message=FALSE, warning=FALSE, results= "hide", fig.align = 'center'}
...
### Table 2. Assessing fit and accuracy for three different variable combinations
...
Of our three logistic regression models, the clinical and physiological model had the lowest AIC score of 320, followed by the clincal model with an AIC score of 330. However, the predictive performance as assessed by AUC was higher for the clinical and physiological model (0.88) than the clinical model (0.86). The clinical and physiologic model also required more parameters for prediction that require blood testing. 
...
fit.cv.bin.model2 <-train(pcos_y_n ~ ., 
                          trControl = ctrl,
                                 data = train.data.clinical, method = "glmnet",
                                 lambda= 0,
                                 tuneGrid = expand.grid(alpha = 0,  
                                                        lambda = 0),
                                 verbose = FALSE,
...
### 4.1.1 - Assessing penalized logistic regression with cross-validation for (RIDGE and LASSO), and Elastic Net (comparison of regularization methods)
...
train.data.clinical <- train.data.covariates.all %>%
  select(
    pcos_y_n,
    weight_gain_y_n ,
    hair_growth_y_n,
...
### Table 3. Comparison of regularization methods across two models for predicting PCOS status

```{r echo=FALSE}
knitr::kable(data.frame("Model"=c("Clinical and Physiologic", "Clinical", 
                     "Clinical and Physiologic", "Clinical", 
                     "Clinical and Physiologic", "Clinical" 
),
           "Method"=c("ElasticNet", "ElasticNet", "LASSO", "LASSO", "Ridge", "Ridge"),
           "ROC"=c(mean(fit.cv.bin.elastic.model_2$results[,3]), 
                   mean(fit.cv.bin.elastic.model_3$results[,3]), 
                   mean(fit.cv.bin.model.all.lasso$results[,3]),
                   mean(fit.cv.bin.model2.lasso$results[,3]), 
                   mean(fit.cv.bin.model.all$results[,3]),
                   mean(fit.cv.bin.model2$results[,3])
                   )))
```

ElasticNet appeared to outperform both LASSO and Ridge regression in terms of classification performance. We selected ElasticNet as the top performer among regularization methods and compared it with boosting and tree-based methods.  
...
#### Figure 3. Comparative performance on the training data

```{r echo=FALSE, include=TRUE, fig.width=6, fig.height=4, fig.align = 'center'}
model_list <- list(RF.Clinical.Physiological = caret_rf_model3,
                   ...
                   ElasticNet.Clinical.Physiological=fit.cv.bin.elastic.model_2, 
                   ElasticNet.Clinical=fit.cv.bin.elastic.model_3  
...
### Table 4. Comparative performance of two models across three different modelling methodologies on the test data
...
```{r echo=FALSE}
row <-  
  data.frame("Model"="ElasticNet: Clinical Parameters", 
             "Accuracy"=round(as.numeric(mean(fit.cv.bin.elastic.model_3$results[,3])),4), 
             "Sensitivity"=round(as.numeric(mean(fit.cv.bin.elastic.model_3$results[,4])),4), 
             "Specificity"=round(as.numeric(mean(fit.cv.bin.elastic.model_3$results[,5])),4))

row2 <-  data.frame("Model"="ElasticNet: Clinical and Physiological Parameters",
                    "Accuracy"=round(as.numeric(mean(fit.cv.bin.elastic.model_2$results[,3])),4), 
                    "Sensitivity"=round(as.numeric(mean(fit.cv.bin.elastic.model_2$results[,4])),4), 
                    "Specificity"=round(as.numeric(mean(fit.cv.bin.elastic.model_2$results[,5])),4))
...
knitr::kable(df%>%select(Model, Accuracy, Sensitivity, Specificity))
```

## 5. Discussion and Conclusion
### 5.1 Discussion

The tested modelling strategies produced a wide variability in accuracy, sensitivity, and specificity. While we observed higher sensitivity when using ElasticNet to fit the models, regardless of whether physiologic variables were included, XGBoost and Random Forest methods were better suited to optimizing specificity, and tended to have similar accuracy when compared to ElasticNet methods.The model that best balanced accuracy, specificity and clinical utility was the Random Forest model with clinical parameters (n=5 clinical symptoms: weight gain, hair growth + skin darkening, hair loss and pimples). This model performed with the highest accuracy, second highest specificity, and incorporated the simplest set of parameters to make predictions. The latter was an important consideration for our model, given our desire for the model to be easily deployed in resource poor settings to serve populations with limited healthcare access, such as those in southern India. Our model's performance is supported by recent commentaries in the literature that indicate Random Forest models perform the most accurately on a wide variety of datasets^6^. 

One of the drawbacks of the Random Forest is its black box nature, making it harder to determine and/or interpret how the model is making its predictions. This could pose a challenge to clinicians trying to troubleshoot or improve upon the model. Our model is simple and requires few parameters, which may offset this inherent lack of interpretability. An additional advantage of our clinical model is the fact that we do not require the collection and storage of physiological parameters such as the hormones human chorionic gonadotropin (HCG) and anti-mullerian hormone (AMH), which may be used as indicators/proxies to determine pregnancy status and fertility respectively. This is sensitive data to patients and limiting the scope and invasiveness of data collection allows for better control of risk of harm to patients, as less data is vulnerable to access by predatory actors.

### 5.2 Conclusions

PCOS status can be predicted with a reasonably high level of accuracy (88.1% with our model) using a clinical parameters that are easily measurable in a general clinical practice setting. By optimizing for specificity, patients likely to be PCOS positive can be selected for more expensive and invasive testing, thus reducing the cost of care and improving patient experience for the population of at-risk individuals Implementation of model-based screening should be considered particularly in resource-limited settings, where access to advanced diagnostics is challenging due to cost or geography. 
...
8. Link for PCOS dataset on Kaggle: https://www.kaggle.com/datasets/prasoonkottarathil/polycystic-ovary-syndrome-pcos
...
#### Appendix plot 2, (EDA), bivariate analysis examining correlations between continuous features 
...
plot_boxplot(PCOS_data_without_infertility_formatted, by = "pcos_y_n", title = "Boxplots for continous variables by PCOS status", ggtheme = theme_bw())



```

* Boxplots showing the distributions of continuous variables in our dataset, stratified by PCOS status. These plots help us appreciate visually, based on the PCOS variable, whether or not there appears to be a significant difference for a given continuous variable. 
...
```{r echo=FALSE, message=FALSE, warning=FALSE, results = "hide"}
## cross-validation (WITHOUT RIDGE or LASSO)
...
#### Appendix Plot 4. ROC curves: physiological model (red) vs. clinical model (blue) vs. clinical and physiologic model (green)

```{r echo=FALSE, fig.height=3, fig.width=3, message=FALSE, warning=FALSE, , fig.align = 'center'}
...
```
* ROC plot showing the ROC curves for our simple logistic regression modelling using three different variable selections (based on clinical variables only, physiological variables only, and both clinical and physiological variables). 

### Appendix Plot 5 - Variable importance (based on Random Forest model)
```{r echo=FALSE, message=FALSE, warning = FALSE, results = "hide"}
vip::vip(caret_rf_model3)+theme_bw()
```

* Plot showing variable importance based on random forest modelling. We decided to adopt an approach of utilizing clinical and contextual knowledge for variable selection, and therefore, based on this we did not drop or remove any of the variables we deemed relevant for our modelling. 