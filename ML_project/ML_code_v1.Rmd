...
## Methods
### 1.0 Splitting our dataset into training and validation datasets
...
### 1.1 Building logistic regression models 
...
* We can also use the likelihood ratio test to compare our models. 
...
### 1.2 Model cross-validation (using the caret package)
...
### 1.3 Assessing model classification performance 
...
``` {r penalized_regression echo=FALSE, fig.height=8, fig.width=4, message=FALSE, warning=FALSE}
...
train.data.covariates.1 <- train.data %>%
  ...
  mutate_at(c("pcos_y_n",
              ...
              "skin_darkening_y_n",
              "hair_loss_y_n",
              "pimples_y_n"),as.factor) %>% mutate(
  amh_ng_m_l = as.numeric(amh_ng_m_l))

glimpse(train.data.covariates.1)

set.seed(504)
ridge_caret<- train(train.data.covariates.1[,-1],train.data.covariates.1[,1], method = "glmnet",
                trControl=cv,
...
# saveRDS(train.data.covariates.1, file = "train_data.rds")
...
caret_rf <- train(
  pcos_labelled ~ .,
  data = select(train.data.covariates, -pcos_y_n),                         
  method = "ranger",
  metric = "ROC",
...
caret_xgb <- caret::train(pcos_y_n ~., data = select(train.data.covariates, -pcos_labelled),
                         method = "xgbTree",
                         metric = "ROC",
                         tuneGrid=xgb_grid_1,
