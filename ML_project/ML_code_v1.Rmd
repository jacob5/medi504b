...
### 1.2 Data
The data was acquired from 10 different hospitals in Kerala, India. 
...
* Try different models, and validate and select the model which most accurately predicts PCOS status.
Therefore our overarching research question is: what factor or factors predict PCOS status with the highest accuracy?
...
train.data.1 <- train.data.covariates.1 %>%
                  drop_na() %>%
                  mutate(pcos_y_n = ifelse(pcos_y_n == "1", "Yes", "No"))
...
baselinevars <- names(dplyr::select(train.data.1, 
                         !c(pcos_y_n)))
baselinevars

model_formula_1 <- as.formula(paste("pcos_y_n~ ", 
                               paste(baselinevars, 
                                     collapse = "+")))
...
fit.cv.bin.model2 <-train(pcos_y_n ~ ., trControl = ctrl,
                                 data = train.data.1, method = "glmnet",
                                 lambda= 0,
                                 tuneGrid = expand.grid(alpha = 0,  
                                                        lambda = 0),
                                 verbose = FALSE,
...
fit.cv.bin.model3 <-train(pcos_y_n ~ ., trControl = ctrl,
                                 data = train.data.1, method = "glmnet",
                                 lambda= 0,
                                 tuneGrid = expand.grid(alpha = 0,  
                                                        lambda = 0),
...
fit.cv.con_model1 <-train(pcos_y_n ~ ., trControl = cv_of_data,
                           data = train.data, method = "glmnet",
                           lambda= 0,
...
fit.cv.con_model2 <-train(pcos_y_n ~ i_beta_hcg_m_iu_m_l + fsh_m_iu_m_l + lh_m_iu_m_l + tsh_m_iu_l + amh_ng_m_l, trControl = cv_of_data, data = train.data, method = "glmnet",
                           lambda= 0,
                           tuneGrid = expand.grid(alpha = 1,  
                                                  lambda = 0),
                           verbose = FALSE)
...
fit.cv.con_model3 <- train(pcos_y_n ~ i_beta_hcg_m_iu_m_l + fsh_m_iu_m_l + lh_m_iu_m_l + tsh_m_iu_l + amh_ng_m_l +
                              weight_gain_y_n + hair_growth_y_n + skin_darkening_y_n + hair_loss_y_n + pimples_y_n, trControl = cv_of_data, data = train.data, method = "glmnet",
                           lambda= 0,
                           tuneGrid = expand.grid(alpha = 1,  
                                                  lambda = 0),
                           verbose = FALSE)
