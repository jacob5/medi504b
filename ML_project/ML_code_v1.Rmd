...
## Objectives
Developing a model which uses physical and clinical predictors to diagnose PCOS. 
```{r objectives, echo=FALSE, message=FALSE, warning = FALSE, results = "hide"}
```

## Methods

* Following our EDA, we start by splitting our datset into the training set and the validation set, followed by building a simple model aimed at using our data to find factors that predict PCOS status (our outcome variable)
...
data_loc <- paste0(here::here(), "/ML_project/data/")
...
# reading in PCOS_data_without_infertility from excel file
PCOS_data_without_infertility <-read_excel("PCOS_data_without_infertility.xlsx", sheet = "Full_new")
...
```
* Next in our EDA we sought to identify missing values in our dataset.
* Here, we see a plot showing our variables and the percentage of missing rows per variable.
* This plot shows the missing values in our dataset. Our EDA identified some missing values for two variables: fast food and marriage status. 
* The analysis indicates that only 0.18% of the rows for these variables are missing. Therefore, as this is below the generally used threshold of 5 %, we will simply ignore these missing values for our subsequent analyses. 

* Next, we would like to get a sense of the variation in our dataset. 
* To do this for the continuous variables, we will plot histograms for each continuous variable using the plot_histogram() from the DataExplorer Package. 

### Exploring the distribution of continuous features in our dataset 
...
* These plots allow us to look at the distribution for each of the continuous features in our dataset.
* For instance, we see that the age distribution in our dataset reveals most individuals are in the range of 20 to 40 years. No individuals in the dataset are younger than 20 or older than 50. 
